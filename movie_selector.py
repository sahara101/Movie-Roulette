...
def load_settings():
    ...
    global PLEX_AVAILABLE, JELLYFIN_AVAILABLE, EMBY_AVAILABLE
...
main_window = None
trakt_auth_handler = None

def register_main_window(window):
    global main_window, trakt_auth_handler
    main_window = window
    # Create TraktAuthHandler only after we have the main window
    ...
    try:
        if current_service == 'plex' and PLEX_AVAILABLE:
            ...
            try:
                ...
                cache_file = path_manager.get_path('jellyfin_movies')
            ...
            try:
                # Get total movies from cache
                cache_file = path_manager.get_path('emby_movies')
                if os.path.exists(cache_file):
                    with open(cache_file, 'r') as f:
                        all_movies = json.load(f)
                    total_movies = len(all_movies)
...
def get_emby_id(tmdb_id):
    try:
        ...
        cache_path = path_manager.get_path('emby_movies')
        if not os.path.exists(cache_path):
